...
export async function ListarPedidoPorEmpresa(empresa){
    const comando = `
...
const [resposta] = await con.query(comando, [empresa])
...
export async function ListarPedidosMaisComprados(){
    const comando = `
    ...
    SELECT  	TIPO.ID_TIPO_INGRESSO,
				...
				LOCAL.DS_UF,
				LOCAL.DS_NUM,
                (TIPO.VL_PRECO_TIPO*PDINGRESSO.QTD_ITENS) AS TOTAL,
                PEDIDO.DT_PEDIDO
        ...
        GROUP BY TIPO.ID_TIPO_INGRESSO, DATAS.ID_DATA_INGRESSO, TIPO.NM_TIPO_INGRESSO, EMPRESA.ID_EMPRESA, TIPO.VL_PRECO_TIPO, PDINGRESSO.QTD_ITENS, DATAS.DT_INGRESSO, HORARIOS.DS_HORARIO, LOCAL.DS_LOGRADOURO, LOCAL.DS_BAIRRO, DS_LOCALIDADE, LOCAL.DS_UF, LOCAL.DS_NUM, PEDIDO.DT_PEDIDO
        ORDER BY TOTAL DESC
`
